    ...
    while (getline(in, line)) {
        if (line.at(0) == '*') {
            ...
            if (relation_id == 0) {
                ...
                if (is_insert) {
                    data.on_insert_NATION1(n_nationkey, n_name, n_regionkey, n_comment);
                    data.on_insert_NATION2(n_nationkey, n_name, n_regionkey, n_comment);
                } else {
                    data.on_delete_NATION1(n_nationkey, n_name, n_regionkey, n_comment);
                    data.on_delete_NATION2(n_nationkey, n_name, n_regionkey, n_comment);
                }
            } else if (relation_id == 7) {
                getline(stream, field, '|');
                int r_regionkey = std::stoi(field);
                getline(stream, field, '|');
                StringType r_name = StringType(field);
                getline(stream, field, '|');
                StringType r_comment = StringType(field);

                if (is_insert) {
                    data.on_insert_REGION(r_regionkey, r_name, r_comment);
                } else {
                    data.on_delete_REGION(r_regionkey, r_name, r_comment);
                }
            } else {
                exit(-1);
            }
