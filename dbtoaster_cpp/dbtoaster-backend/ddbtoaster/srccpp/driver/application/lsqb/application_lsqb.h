    ...
    while (getline(in, line)) {
        if (line.at(0) == '*') {
            ...
            if (relation_id == 0) {
                ...
                data.on_insert_PERSON_KNOWS_PERSON1(person1_id, person2_id);
                data.on_insert_PERSON_KNOWS_PERSON2(person1_id, person2_id);
            } else if (relation_id == 1) {
                getline(stream, field, '|');
                unsigned long person_id = std::stoul(field);
                getline(stream, field, '|');
                unsigned long tag_id = std::stoul(field);
                data.on_insert_PERSON_HAS_INTEREST_TAG(person_id, tag_id);
            } else {
                exit(-1);
            }
        }
