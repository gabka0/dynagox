...
commands += Command.command("release")((state: State) => {
  ...
  if (frontendRepo != "") {
    ...
    println("copy application/line5")
    val cppDriverAppLine5Dir = baseDir / "srccpp" / "driver" / "application" / "line5"
    val releaseCppDriverAppLine5Dir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "line5"
    releaseCppDriverAppLine5Dir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppLine5Dir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppLine5Dir)

    println("copy application/dumbbell")
    val cppDriverAppDumbbellDir = baseDir / "srccpp" / "driver" / "application" / "dumbbell"
    val releaseCppDriverAppDumbbellDir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "dumbbell"
    releaseCppDriverAppDumbbellDir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppDumbbellDir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppDumbbellDir)

    println("copy application/job")
    val cppDriverAppJobDir = baseDir / "srccpp" / "driver" / "application" / "job"
    val releaseCppDriverAppJobDir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "job"
    releaseCppDriverAppJobDir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppJobDir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppJobDir)

    println("copy application/ceb")
    val cppDriverAppCebDir = baseDir / "srccpp" / "driver" / "application" / "ceb"
    val releaseCppDriverAppCebDir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "ceb"
    releaseCppDriverAppCebDir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppCebDir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppCebDir)


    println("copy application/lsqb")
    val cppDriverAppLsqbDir = baseDir / "srccpp" / "driver" / "application" / "lsqb"
    val releaseCppDriverAppLsqbDir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "lsqb"
    releaseCppDriverAppLsqbDir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppLsqbDir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppLsqbDir)

    println("copy application/tpch")
    val cppDriverAppTpchDir = baseDir / "srccpp" / "driver" / "application" / "tpch"
    val releaseCppDriverAppTpchDir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "tpch"
    releaseCppDriverAppTpchDir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppTpchDir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppTpchDir)

    println("copy application/tpcds")
    val cppDriverAppTpcdsDir = baseDir / "srccpp" / "driver" / "application" / "tpcds"
    val releaseCppDriverAppTpcdsDir = releaseDir / "lib" / "dbt_c++" / "driver" / "application" / "tpcds"
    releaseCppDriverAppTpcdsDir.mkdirs
    copyFiles(IO.listFiles(cppDriverAppTpcdsDir).filter { f => f.getName.endsWith(".h") || f.getName.endsWith(".hpp") }, releaseCppDriverAppTpcdsDir)
