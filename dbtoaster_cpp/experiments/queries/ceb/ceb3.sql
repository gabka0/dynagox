...
FROM aka_name AS an, cast_info AS ci, movie_info AS mi,
     movie_info_idx1 AS mii1, movie_info_idx2 AS mii2, person_info AS pi
WHERE an.person_id = ci.person_id AND pi.person_id = ci.person_id
  AND ci.movie_id = mi.movie_id AND ci.movie_id = mii1.movie_id AND ci.movie_id = mii2.movie_id;