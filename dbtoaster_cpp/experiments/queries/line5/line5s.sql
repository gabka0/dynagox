CREATE STREAM R1(src double, dst long) FROM FILE 'R1.dat' LINE DELIMITED CSV;
...
CREATE STREAM R5(src long, dst double) FROM FILE 'R5.dat' LINE DELIMITED CSV;

SELECT SUM(R1.src * R5.dst) FROM R1, R2, R3, R4, R5
