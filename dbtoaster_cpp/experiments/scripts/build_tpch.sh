...
for query in "${queries[@]}"; do
  echo "Building ${query}..."
  g++ -O3 -DNDEBUG -Wall -Wno-unused-variable -std=c++14 \
    -pedantic "${RELEASE_PATH}/lib/dbt_c++/driver/main.cpp" \
    -I "${RELEASE_PATH}/lib/dbt_c++/lib/" \
    -I "${RELEASE_PATH}/lib/dbt_c++/driver/" \
    -include "${EXPERIMENTS_PATH}/hpps/${query}.hpp" \
    -include "${RELEASE_PATH}/lib/dbt_c++/driver/application/tpch/application_${query}.h" \
