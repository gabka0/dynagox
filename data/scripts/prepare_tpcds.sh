...
mkdir -p "${DATA_PATH}/tpcds"

echo "Downloading DuckDB..."
rm -f "${DATA_PATH}/tpcds/duckdb_cli-linux-amd64.zip"
wget -P "${DATA_PATH}/tpcds/" https://github.com/duckdb/duckdb/releases/download/v1.1.3/duckdb_cli-linux-amd64.zip
unzip -d "${DATA_PATH}/tpcds/" "${DATA_PATH}/tpcds/duckdb_cli-linux-amd64.zip"
rm -f "${DATA_PATH}/tpcds/duckdb_cli-linux-amd64.zip"

cd "${DATA_PATH}/tpcds"
rm -f "${DATA_PATH}/tpcds/export.sql"
touch "${DATA_PATH}/tpcds/export.sql"

SCALE_FACTOR="10"
echo "Generating tpcds data[SF=${SCALE_FACTOR}]..."
rm -f "${DATA_PATH}/tpcds/*.csv"

echo "INSTALL tpcds;" >> "${DATA_PATH}/tpcds/export.sql"
echo "LOAD tpcds;" >> "${DATA_PATH}/tpcds/export.sql"
echo "CALL dsdgen(sf = ${SCALE_FACTOR});" >> "${DATA_PATH}/tpcds/export.sql"
...
TO 'store_sales.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"
...
TO 'catalog_sales.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"

echo "COPY (SELECT s_store_sk FROM store WHERE s_store_sk IS NOT NULL)
TO 'store.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"
...
TO 'customer.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"

echo "COPY (SELECT ca_address_sk, ca_zip FROM customer_address WHERE ca_address_sk IS NOT NULL AND ca_zip IS NOT NULL)
TO 'customer_address.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"

echo "COPY (SELECT d_date_sk FROM date_dim WHERE d_date_sk IS NOT NULL)
TO 'date_dim.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"
...
TO 'item.csv' (HEADER, DELIMITER '|');" >> "${DATA_PATH}/tpcds/export.sql"

"${DATA_PATH}/tpcds/duckdb" < "${DATA_PATH}/tpcds/export.sql"

rm -f "${DATA_PATH}/tpcds/export.sql"
rm -f "${DATA_PATH}/tpcds/duckdb"
