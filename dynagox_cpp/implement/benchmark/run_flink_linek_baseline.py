...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run Flink SQL baseline for Line(k,a,b) and export CSV."
    )
    ...
    p.add_argument(
        "--work-dir",
        default="/tmp",
        help="Temp working directory for generated SQL and sink files",
    )
    return p.parse_args()


def load_milestones(stream_csv: Path) -> list[tuple[int, int]]:
    marks: list[int] = []
    ...
    if not marks:
        raise ValueError("No milestones found in stream csv")

    windows: list[tuple[int, int]] = []
    prev = -1
    for m in marks:
        # Data ops are rows between previous and current milestone marker rows.
        # Milestone row itself is not a data operation.
        ops = m - prev - 1
...
def var_expr(k: int, idx: int) -> str:
    if idx < 1 or idx > k + 1:
        raise ValueError(f"Invalid variable index x{idx} for k={k}")
    if idx == 1:
        return "r1.src"
    if idx == k + 1:
        return f"r{k}.dst"
...
def build_sql(stream_csv: Path, sink_dir: Path, k: int, a: int, b: int) -> str:
    ...
    for idx in range(a, b + 1):
        ...
        sink_cols.append(f"{alias} BIGINT")
    ...
    select_prefix = ",\n  ".join(["m.phase_idx"] + select_cols + ["COUNT(*) AS cnt"])
    group_by = ", ".join(["m.phase_idx"] + group_cols)
    joins = "\n".join(join_clauses)
    max_seq_expr = ", ".join([f"r{i}.seq" for i in range(1, k + 1)])
...
"""
    for rid in range(1, k + 1):
        sql += (
            ...
            f"SELECT src, dst, seq FROM updates "
            f"WHERE rid = {rid} AND op = '+';\n"
        )
    sql += """
...
"""
    ...
    sql += f"""CREATE TEMPORARY TABLE sink (
  {sink_schema}
) WITH (
  'connector' = 'filesystem',
...
FROM r1
{joins}
JOIN milestones m ON GREATEST({max_seq_expr}) < m.mseq
GROUP BY {group_by};
"""
    return sql


def collect_sink_items_by_phase(
    sink_dir: Path, key_cols: int
    ...
    for phase_idx, items in by_phase.items():
...
def main() -> None:
    ...
    windows = load_milestones(stream_csv)
    ...
    with out_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for idx, (k, a, b) in enumerate(all_combos, start=1):
            print(f"[{idx}/{total}] Running k={k} a={a} b={b}", flush=True)
            key_cols = b - a + 1
            job_dir = Path(
                tempfile.mkdtemp(
                    prefix=f"flink_linek_k{k}_a{a}_b{b}_",
            ...
            if proc.returncode != 0:
                raise RuntimeError(
                    f"Flink query failed for (k={k},a={a},b={b}).\n{proc.stderr}"
                )

            by_phase = collect_sink_items_by_phase(sink_dir, key_cols=key_cols)
            ...
            print(f"[{idx}/{total}] Done k={k} a={a} b={b}", flush=True)
