#ifndef DYNAGOX_CROWN_TPCH5_H
#define DYNAGOX_CROWN_TPCH5_H
...
class CrownTPCH5 : public CrownBase {
private:
  ...
  inline void update_activate_pv_s(unsigned long s_suppkey, unsigned long s_nationkey);

  inline void update_activate_r_c(unsigned long c_custkey, unsigned long c_nationkey);

  inline void update_activate_pv_c(unsigned long c_custkey, unsigned long c_nationkey, const std::string &n_name);

  inline void update_activate_r_o(unsigned long o_orderkey, unsigned long o_custkey);

  inline void update_activate_pv_o(unsigned long o_orderkey, unsigned long c_nationkey, const std::string &n_name);
  ...
  inline void update_activate_pv_n(unsigned long n_nationkey, const std::string &n_name);

  inline void update_activate_r_r(unsigned long r_regionkey);

  inline void update_add_sv_gl(const std::string &n_name, double value);
  ...
  MultiHashMap<SupplierJoinKeyTPCH5, PrimaryHashIndex<SupplierJoinKeyTPCH5, SupplierJoinKeyTPCH5Hash,
      SupplierJoinKeyTPCH5Equal>> pv_s;
  ...
  MultiHashMap<CustomerJoinKeyTPCH5, PrimaryHashIndex<CustomerJoinKeyTPCH5, CustomerJoinKeyTPCH5Hash,
      CustomerJoinKeyTPCH5Equal>> pv_c;
  ...
  MultiHashMap<NationJoinKeyTPCH5, PrimaryHashIndex<NationJoinKeyTPCH5, NationJoinKeyTPCH5Hash,
      NationJoinKeyTPCH5Equal>> pv_n;

  MultiHashMap<RegionTPCH5, PrimaryHashIndex<RegionTPCH5, RegionTPCH5Hash, RegionTPCH5Equal>> r_r;

  MultiHashMap<GLineitemTPCH5, PrimaryHashIndex<GLineitemTPCH5, GLineitemTPCH5Hash, GLineitemTPCH5Equal>> sv_gl;

public:
  explicit CrownTPCH5(std::ostream &output, bool print_result, bool delta_mode) : CrownBase(output, print_result),
