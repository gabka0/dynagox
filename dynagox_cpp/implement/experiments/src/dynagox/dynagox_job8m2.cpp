...
void DynaGoxJOB8m2::process(const std::string &line) {
...
void DynaGoxJOB8m2::milestone() { print(); }

void DynaGoxJOB8m2::insert_into_an(unsigned long id, unsigned long person_id) {
  ank.f0 = person_id;
...
void DynaGoxJOB8m2::insert_into_ci(unsigned long id, unsigned long person_id,
...
void DynaGoxJOB8m2::insert_into_mc(unsigned long id, unsigned long movie_id) {
  mck.f0 = movie_id;
  auto d_mc = apv_mc.update_max2_approx(mck, static_cast<double>(id), b);
  if (d_mc != 0) {
    cit.f1 = movie_id;
