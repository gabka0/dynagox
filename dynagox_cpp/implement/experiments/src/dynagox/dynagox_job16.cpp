...
void DynaGoxJOB16::process(const std::string &line) {
...
void DynaGoxJOB16::milestone() { print(); }

void DynaGoxJOB16::insert_into_an(unsigned long id, unsigned long person_id) {
  ank.f0 = person_id;
...
void DynaGoxJOB16::insert_into_ci(unsigned long id, unsigned long person_id,
  ...
  if (kmc != nullptr) {
    mkk.f0 = movie_id;
    auto kmk = apv_mk.get(mkk);
    if (kmk != nullptr) {
      cik.f0 = person_id;
      apv_ci.update_add(cik, kmc->approximate * kmk->approximate);

      ank.f0 = person_id;
      auto kan = apv_an.get(ank);
      if (kan != nullptr) {
        result += (kan->annotation * kmc->approximate * kmk->approximate);
      }
    }
  }
}

void DynaGoxJOB16::insert_into_mc(unsigned long id, unsigned long movie_id) {
  mck.f0 = movie_id;
...
void DynaGoxJOB16::insert_into_mk(unsigned long id, unsigned long movie_id) {
  mkk.f0 = movie_id;
  auto delta_pv_mk = apv_mk.update_add_approx(mkk, 1, b);
  if (delta_pv_mk != 0) {
    mck.f0 = movie_id;
    auto kmc = apv_mc.get(mck);
    if (kmc != nullptr) {
