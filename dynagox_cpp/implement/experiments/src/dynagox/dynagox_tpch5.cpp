...
void DynaGoxTPCH5::process(const std::string &line) {
...
void DynaGoxTPCH5::milestone() {
  if (!delta_mode) {
    auto gk_gl = hv_gl.get_keys();
    while (gk_gl != nullptr) {
...
void DynaGoxTPCH5::insert_into_supplier(unsigned long s_suppkey, unsigned long s_nationkey) {
  update_activate_r_s(s_suppkey, s_nationkey);
  k_n.n_nationkey = s_nationkey;
  auto g_n = apv_n.get(k_n);
  if (g_n != nullptr) {
...
void DynaGoxTPCH5::insert_into_customer(unsigned long c_custkey, unsigned long c_nationkey) {
...
void DynaGoxTPCH5::insert_into_orders(unsigned long o_orderkey, unsigned long o_custkey, unsigned long o_orderdate) {
...
void DynaGoxTPCH5::insert_into_lineitem(unsigned long l_orderkey, unsigned long l_suppkey, unsigned long l_linenumber,
...
void DynaGoxTPCH5::insert_into_nation(unsigned long n_nationkey, const std::string &n_name, unsigned long n_regionkey) {
...
void DynaGoxTPCH5::insert_into_region(unsigned long r_regionkey, std::string &r_name) {
  if (r_name == "ASIA") {
...
void DynaGoxTPCH5::print(const std::string &n_name, double value) {
  if (print_result) {
    fmt::println(output, "({}) -> {:.4f}", n_name, value);
  }
}

void DynaGoxTPCH5::update_activate_r_s(unsigned long s_suppkey, unsigned long s_nationkey) {
  t_s.s_suppkey = s_suppkey;
  t_s.s_nationkey = s_nationkey;
  r_s.update_activate(t_s);
}

void DynaGoxTPCH5::update_activate_apv_s(unsigned long s_suppkey, unsigned long s_nationkey) {
  k_s.s_suppkey = s_suppkey;
  k_s.s_nationkey = s_nationkey;
  apv_s.update_activate(k_s);
}

void DynaGoxTPCH5::update_activate_r_c(unsigned long c_custkey, unsigned long c_nationkey) {
  t_c.c_custkey = c_custkey;
  t_c.c_nationkey = c_nationkey;
  r_c.update_activate(t_c);
}

void DynaGoxTPCH5::update_activate_apv_c(unsigned long c_custkey, unsigned long c_nationkey, const std::string &n_name) {
...
void DynaGoxTPCH5::update_activate_r_o(unsigned long o_orderkey, unsigned long o_custkey) {
...
void
DynaGoxTPCH5::update_activate_apv_o(unsigned long o_orderkey, unsigned long c_nationkey, const std::string &n_name) {
...
void DynaGoxTPCH5::update_activate_r_l(unsigned long l_orderkey, unsigned long l_suppkey, unsigned long l_linenumber,
                                       double l_extendedprice, double l_discount) {
...
void
DynaGoxTPCH5::update_activate_r_n(unsigned long n_nationkey, const std::string &n_name, unsigned long n_regionkey) {
...
void DynaGoxTPCH5::update_activate_apv_n(unsigned long n_nationkey, const std::string &n_name) {
  k_n.n_nationkey = n_nationkey;
  k_n.n_name = n_name;
  apv_n.update_activate(k_n);
}

void DynaGoxTPCH5::update_activate_r_r(unsigned long r_regionkey) {
  t_r.r_regionkey = r_regionkey;
  r_r.update_activate(t_r);
}

void DynaGoxTPCH5::update_add_hv_gl(const std::string &n_name, double value) {
  t_gl.n_name = n_name;
