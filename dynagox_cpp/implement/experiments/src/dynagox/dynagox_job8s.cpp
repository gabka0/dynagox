...
void DynaGoxJOB8s::process(const std::string &line) {
...
void DynaGoxJOB8s::milestone() { print(); }

void DynaGoxJOB8s::insert_into_an(unsigned long id, unsigned long person_id) {
  ank.f0 = person_id;
  apv_an.update_add(ank, static_cast<double>(id));
...
void DynaGoxJOB8s::insert_into_ci(unsigned long id, unsigned long person_id,
...
void DynaGoxJOB8s::insert_into_mc(unsigned long id, unsigned long movie_id) {
  mck.f0 = movie_id;
  auto delta_pv_mc = apv_mc.update_add_approx(mck, static_cast<double>(id), b);
  if (delta_pv_mc != 0) {
    cit.f1 = movie_id;
