...
void CrownTPCH8::process(const std::string &line) {
...
void CrownTPCH8::milestone() {
  if (!delta_mode) {
    auto gk_go2 = sv_go2.get_keys();
    while (gk_go2 != nullptr) {
      auto o_year = gk_go2->o_year;
      auto value2 = gk_go2->annotation;

      t_go.o_year = o_year;
      auto g_go1 = sv_go1.get(t_go);
...
void CrownTPCH8::insert_into_part(unsigned long p_partkey, const std::string &p_type) {
    ...
    while (f_l != nullptr) {
      ...
      if (g_s != nullptr) {
        ...
        if (g_s->n_name == "BRAZIL") {
          update_add_pv_l1(f_l->object->l_orderkey, volume);
        }
        update_add_pv_l2(f_l->object->l_orderkey, volume);

        k_o.o_orderkey = f_l->object->l_orderkey;
        auto g_o = pv_o.get(k_o);
        if (g_o != nullptr) {
          if (g_s->n_name == "BRAZIL") {
            update_add_sv_go1(g_o->o_year, volume);
          }
          update_add_sv_go2(g_o->o_year, volume);

          if (delta_mode) {
            t_go.o_year = g_o->o_year;
            auto g_go1 = sv_go1.get(t_go);
            auto value1 = g_go1 != nullptr ? g_go1->annotation : 0;
            auto g_go2 = sv_go2.get(t_go);
...
void CrownTPCH8::insert_into_supplier(unsigned long s_suppkey, unsigned long s_nationkey) {
  update_activate_r_s(s_suppkey, s_nationkey);
  t_n2.n_nationkey = s_nationkey;
  auto g_n2 = r_n2.get(t_n2);
  if (g_n2 != nullptr) {
    update_activate_pv_s(s_suppkey, g_n2->n_name);
...
void CrownTPCH8::insert_into_customer(unsigned long c_custkey, unsigned long c_nationkey) {
  update_activate_r_c(c_custkey, c_nationkey);
  k_n1.n_nationkey = c_nationkey;
  auto g_n1 = pv_n1.get(k_n1);
  if (g_n1 != nullptr) {
    update_activate_pv_c(c_custkey);
    t_o.o_custkey = c_custkey;
    auto f_o = r_o.find(0, t_o);
    while (f_o != nullptr) {
      update_activate_pv_o(f_o->object->o_orderkey, f_o->object->o_year);
      k_l.l_orderkey = f_o->object->o_orderkey;
      auto g_l1 = pv_l1.get(k_l);
      if (g_l1 != nullptr) {
        update_add_sv_go1(f_o->object->o_year, g_l1->annotation);
      }

      auto g_l2 = pv_l2.get(k_l);
      if (g_l2 != nullptr) {
        update_add_sv_go2(f_o->object->o_year, g_l2->annotation);
...
void CrownTPCH8::insert_into_orders(unsigned long o_orderkey, unsigned long o_custkey, unsigned long o_orderdate,
  ...
  if (19950101 <= o_orderdate && o_orderdate <= 19961231) {
    ...
    if (g_c != nullptr) {
      update_activate_pv_o(o_orderkey, o_year);
      k_l.l_orderkey = o_orderkey;
      auto g_l1 = pv_l1.get(k_l);
      if (g_l1 != nullptr) {
        update_add_sv_go1(o_year, g_l1->annotation);
      }

      auto g_l2 = pv_l2.get(k_l);
      if (g_l2 != nullptr) {
        update_add_sv_go2(o_year, g_l2->annotation);
...
void CrownTPCH8::insert_into_lineitem(unsigned long l_orderkey, unsigned long l_partkey, unsigned long l_suppkey,
  ...
  if (g_p != nullptr) {
    ...
    if (g_s != nullptr) {
      ...
      if (g_s->n_name == "BRAZIL") {
        update_add_pv_l1(l_orderkey, volume);
      }
      update_add_pv_l2(l_orderkey, volume);
...
void CrownTPCH8::insert_into_nation1(unsigned long n_nationkey, unsigned long n_regionkey) {
  update_activate_r_n1(n_nationkey, n_regionkey);
  t_r.r_regionkey = n_regionkey;
  auto g_r = r_r.get(t_r);
  if (g_r != nullptr) {
    update_activate_pv_n1(n_nationkey);
    t_c.c_nationkey = n_nationkey;
    auto f_c = r_c.find(0, t_c);
    while (f_c != nullptr) {
      update_activate_pv_c(f_c->object->c_custkey);
...
void CrownTPCH8::insert_into_nation2(unsigned long n_nationkey, const std::string &n_name) {
...
void CrownTPCH8::insert_into_region(unsigned long r_regionkey, std::string &r_name) {
  if (r_name == "AMERICA") {
    update_activate_r_r(r_regionkey);
    t_n1.n_regionkey = r_regionkey;
    auto f_n1 = r_n1.find(0, t_n1);
    while (f_n1 != nullptr) {
      update_activate_pv_n1(f_n1->object->n_nationkey);
...
void CrownTPCH8::print(unsigned long o_year, double value1, double value2) {
  if (print_result) {
    fmt::println(output, "({}) -> ({:.4f},{:.4f})", o_year, value1, value2);
  }
}

void CrownTPCH8::update_activate_r_p(unsigned long p_partkey) {
  t_p.p_partkey = p_partkey;
  r_p.update_activate(t_p);
}

void CrownTPCH8::update_activate_r_s(unsigned long s_suppkey, unsigned long s_nationkey) {
  t_s.s_suppkey = s_suppkey;
  t_s.s_nationkey = s_nationkey;
  r_s.update_activate(t_s);
}

void CrownTPCH8::update_activate_pv_s(unsigned long s_suppkey, const std::string &n_name) {
  k_s.s_suppkey = s_suppkey;
  k_s.n_name = n_name;
  pv_s.update_activate(k_s);
}

void CrownTPCH8::update_activate_r_c(unsigned long c_custkey, unsigned long c_nationkey) {
  t_c.c_custkey = c_custkey;
  t_c.c_nationkey = c_nationkey;
  r_c.update_activate(t_c);
}

void CrownTPCH8::update_activate_pv_c(unsigned long c_custkey) {
  k_c.c_custkey = c_custkey;
  pv_c.update_activate(k_c);
}

void CrownTPCH8::update_activate_r_o(unsigned long o_orderkey, unsigned long o_custkey, unsigned long o_year) {
...
void CrownTPCH8::update_activate_pv_o(unsigned long o_orderkey, unsigned long o_year) {
  k_o.o_orderkey = o_orderkey;
  k_o.o_year = o_year;
  pv_o.update_activate(k_o);
}

void CrownTPCH8::update_activate_r_l(unsigned long l_orderkey, unsigned long l_partkey, unsigned long l_suppkey,
                                     unsigned long l_linenumber, double l_extendedprice, double l_discount) {
...
void CrownTPCH8::update_add_pv_l1(unsigned long l_orderkey, double value) {
  k_l.l_orderkey = l_orderkey;
  pv_l1.update_add(k_l, value);
}

void CrownTPCH8::update_add_pv_l2(unsigned long l_orderkey, double value) {
  k_l.l_orderkey = l_orderkey;
  pv_l2.update_add(k_l, value);
}

void CrownTPCH8::update_activate_r_n1(unsigned long n_nationkey, unsigned long n_regionkey) {
  t_n1.n_nationkey = n_nationkey;
  t_n1.n_regionkey = n_regionkey;
  r_n1.update_activate(t_n1);
}

void CrownTPCH8::update_activate_pv_n1(unsigned long n_nationkey) {
  k_n1.n_nationkey = n_nationkey;
  pv_n1.update_activate(k_n1);
}

void CrownTPCH8::update_activate_r_n2(unsigned long n_nationkey, const std::string &n_name) {
  t_n2.n_nationkey = n_nationkey;
  t_n2.n_name = n_name;
  r_n2.update_activate(t_n2);
}

void CrownTPCH8::update_activate_r_r(unsigned long r_regionkey) {
  t_r.r_regionkey = r_regionkey;
  r_r.update_activate(t_r);
}

void CrownTPCH8::update_add_sv_go1(unsigned long o_year, double value) {
  t_go.o_year = o_year;
  sv_go1.update_add(t_go, value);
}

void CrownTPCH8::update_add_sv_go2(unsigned long o_year, double value) {
  t_go.o_year = o_year;
  sv_go2.update_add(t_go, value);
}