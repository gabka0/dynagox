...
void CrownTPCDS15::process(const std::string &line) {
...
void CrownTPCDS15::milestone() {
  if (!delta_mode) {
    auto gk_gc = sv_gc.get_keys();
    while (gk_gc != nullptr) {
...
void CrownTPCDS15::insert_into_catalog_sales(unsigned long cs_sold_date_sk, unsigned long cs_bill_customer_sk,
  ...
  if (g_d != nullptr) {
    ...
    pv_cs.update_add(k_cs, cs_sales_price);

    k_c.c_customer_sk = cs_bill_customer_sk;
    auto g_c = pv_c.get(k_c);
    if (g_c != nullptr) {
      t_gc.ca_zip = g_c->ca_zip;
      sv_gc.update_add(t_gc, cs_sales_price);
      if (delta_mode) {
        auto g_gc = sv_gc.get(t_gc);
...
void CrownTPCDS15::insert_into_customer(unsigned long c_customer_sk, unsigned long c_current_addr_sk) {
  ...
  if (g_ca != nullptr) {
    ...
    auto g_cs = pv_cs.get(k_cs);
    if (g_cs != nullptr) {
      t_gc.ca_zip = g_ca->ca_zip;
      sv_gc.update_add(t_gc, g_cs->annotation);
      if (delta_mode) {
        auto g_gc = sv_gc.get(t_gc);
        print(g_gc->ca_zip, g_gc->annotation);
      }
...
void CrownTPCDS15::insert_into_customer_address(unsigned long ca_address_sk, const std::string &ca_zip) {
...
void CrownTPCDS15::insert_into_date_dim(unsigned long d_date_sk) {
  ...
  while (f_cs != nullptr) {
    ...
    pv_cs.update_add(k_cs, f_cs->object->cs_sales_price * f_cs->object->annotation);

    k_c.c_customer_sk = f_cs->object->cs_bill_customer_sk;
    auto g_c = pv_c.get(k_c);
    if (g_c != nullptr) {
      t_gc.ca_zip = g_c->ca_zip;
      sv_gc.update_add(t_gc, f_cs->object->cs_sales_price * f_cs->object->annotation);
      if (delta_mode) {
        auto g_gc = sv_gc.get(t_gc);
        print(g_gc->ca_zip, g_gc->annotation);
      }
...
void CrownTPCDS15::print(const std::string &ca_zip, double value) {
  if (print_result) {
