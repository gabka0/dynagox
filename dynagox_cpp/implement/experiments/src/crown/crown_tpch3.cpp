...
void CrownTPCH3::process(const std::string &line) {
...
void CrownTPCH3::insert_into_customer(unsigned long c_custkey, const std::string &c_mktsegment) {
  if (c_mktsegment == "BUILDING") {
    update_activate_r_c(c_custkey);
    t_o.o_custkey = c_custkey;
    auto f_o = r_o.find(0, t_o);
    while (f_o != nullptr) {
      update_activate_pv_o(f_o->object->o_orderkey, f_o->object->o_orderdate, f_o->object->o_shippriority);
      k_l.l_orderkey = f_o->object->o_orderkey;
      auto g_l = pv_l.get(k_l);
      if (g_l != nullptr) {
        update_add_sv_go(f_o->object->o_orderkey, f_o->object->o_orderdate, f_o->object->o_shippriority,
                         g_l->annotation);
        if (delta_mode) {
          print(f_o->object->o_orderkey, f_o->object->o_orderdate, f_o->object->o_shippriority, g_l->annotation);
        }
      }
      f_o = f_o->next;
    }
...
void CrownTPCH3::delete_from_customer(unsigned long c_custkey, const std::string &c_mktsegment) {
  if (c_mktsegment == "BUILDING") {
    ...
    while (f_o != nullptr) {
      ...
      auto g_l = pv_l.get(k_l);
      if (g_l != nullptr) {
        update_subtract_sv_go(f_o->object->o_orderkey, g_l->annotation);
        if (delta_mode) {
...
void CrownTPCH3::insert_into_orders(unsigned long o_orderkey, unsigned long o_custkey, unsigned long o_orderdate,
  ...
  if (o_orderdate < 19950315) {
    ...
    if (g_c != nullptr) {
      update_activate_pv_o(o_orderkey, o_orderdate, o_shippriority);
      k_l.l_orderkey = o_orderkey;
      auto g_l = pv_l.get(k_l);
      if (g_l != nullptr) {
        update_add_sv_go(o_orderkey, o_orderdate, o_shippriority, g_l->annotation);
        if (delta_mode) {
          print(o_orderkey, o_orderdate, o_shippriority, g_l->annotation);
        }
      }
    }
  }
...
void CrownTPCH3::delete_from_orders(unsigned long o_orderkey, unsigned long o_custkey, unsigned long o_orderdate,
                                    unsigned long o_shippriority) {
  if (o_orderdate < 19950315) {
    ...
    if (g_c != nullptr) {
      ...
      auto g_l = pv_l.get(k_l);
      if (g_l != nullptr) {
        update_subtract_sv_go(o_orderkey, g_l->annotation);
        if (delta_mode) {
...
void CrownTPCH3::insert_into_lineitem(unsigned long l_orderkey, double l_extendedprice, double l_discount,
  ...
  if (l_shipdate > 19950315) {
    ...
    if (g_o != nullptr) {
      update_add_sv_go(g_o->o_orderkey, g_o->o_orderdate, g_o->o_shippriority, l_extendedprice * (1 - l_discount));
      if (delta_mode) {
...
void CrownTPCH3::delete_from_lineitem(unsigned long l_orderkey, double l_extendedprice, double l_discount,
                                      unsigned long l_shipdate) {
  if (l_shipdate > 19950315) {
    ...
    auto g_o = pv_o.get(k_o);
    if (g_o != nullptr) {
      update_subtract_sv_go(g_o->o_orderkey, l_extendedprice * (1 - l_discount));
      if (delta_mode) {
        t_go.o_orderkey = l_orderkey;
        auto g_go = sv_go.get(t_go);
        if (g_go != nullptr) {
...
void CrownTPCH3::print(unsigned long o_orderkey, unsigned long o_orderdate, unsigned long o_shippriority,
                       double value) {
  if (print_result) {
...
void CrownTPCH3::update_activate_r_c(unsigned long c_custkey) {
  t_c.c_custkey = c_custkey;
  r_c.update_activate(t_c);
}

void CrownTPCH3::update_remove_r_c(unsigned long c_custkey) {
  t_c.c_custkey = c_custkey;
  r_c.update_remove_if_exists(t_c);
}

void CrownTPCH3::update_activate_r_o(unsigned long o_orderkey, unsigned long o_custkey, unsigned long o_orderdate,
                                     unsigned long o_shippriority) {
...
void CrownTPCH3::update_remove_r_o(unsigned long o_orderkey) {
  t_o.o_orderkey = o_orderkey;
  r_o.update_remove_if_exists(t_o);
}

void CrownTPCH3::update_activate_pv_o(unsigned long o_orderkey, unsigned long o_orderdate,
                                      unsigned long o_shippriority) {
...
void CrownTPCH3::update_remove_pv_o(unsigned long o_orderkey) {
  k_o.o_orderkey = o_orderkey;
  pv_o.update_remove_if_exists(k_o);
}

void CrownTPCH3::update_add_pv_l(unsigned long l_orderkey, double value) {
  k_l.l_orderkey = l_orderkey;
  pv_l.update_add(k_l, value);
}

void CrownTPCH3::update_subtract_pv_l(unsigned long l_orderkey, double value) {
  k_l.l_orderkey = l_orderkey;
  pv_l.update_subtract(k_l, value);
}

void CrownTPCH3::update_add_sv_go(unsigned long o_orderkey, unsigned long o_orderdate, unsigned long o_shippriority,
                                  double value) {
...
void CrownTPCH3::update_subtract_sv_go(unsigned long o_orderkey, double value) {
  t_go.o_orderkey = o_orderkey;
