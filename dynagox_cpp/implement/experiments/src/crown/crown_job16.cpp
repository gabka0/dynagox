...
void CrownJOB16::process(const std::string &line) {
...
void CrownJOB16::milestone() { print(); }

void CrownJOB16::insert_into_an(unsigned long id, unsigned long person_id) {
  ank.f0 = person_id;
...
void CrownJOB16::insert_into_ci(unsigned long id, unsigned long person_id,
  ...
  if (kmc != nullptr) {
    mkk.f0 = movie_id;
    auto kmk = pv_mk.get(mkk);
    if (kmk != nullptr) {
      cik.f0 = person_id;
      pv_ci.update_add(cik, kmc->annotation * kmk->annotation);

      ank.f0 = person_id;
      auto kan = pv_an.get(ank);
      if (kan != nullptr) {
        result += (kan->annotation * kmc->annotation * kmk->annotation);
      }
    }
  }
}

void CrownJOB16::insert_into_mc(unsigned long id, unsigned long movie_id) {
  mck.f0 = movie_id;
  ...
  if (kmk != nullptr) {
    ...
    while (tuple_ci != nullptr) {
      ...
      pv_ci.update_add(cik, tuple_ci->object->annotation * kmk->annotation);

      ank.f0 = tuple_ci->object->f0;
      auto kan = pv_an.get(ank);
      if (kan != nullptr) {
        result +=
            (kan->annotation * tuple_ci->object->annotation * kmk->annotation);
      }
...
void CrownJOB16::insert_into_mk(unsigned long id, unsigned long movie_id) {
  mkk.f0 = movie_id;
  pv_mk.update_add(mkk, 1);
