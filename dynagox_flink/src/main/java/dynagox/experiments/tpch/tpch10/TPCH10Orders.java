...
public class TPCH10Orders implements MultiHashMapKey {
    public int o_orderkey;
    public int o_custkey;
    private double annotation;
    private double approximate;
    private boolean active;
    private TPCH10Orders previous;
    private TPCH10Orders next;
    ...
    public void set_previous(MultiHashMapKey previous) {
        this.previous = (TPCH10Orders) previous;
    }

    @Override
    public MultiHashMapKey copy() {
        TPCH10Orders result = new TPCH10Orders();
    ...
    public static class Hash0 implements HashFunction<TPCH10Orders> {
        @Override
        public int hash(TPCH10Orders key) {
            return (key.o_orderkey + 0x9e3779b9);
        }
    }

    public static class Equal0 implements EqualFunction<TPCH10Orders> {
        @Override
        public boolean equal(TPCH10Orders a, TPCH10Orders b) {
            return a.o_orderkey == b.o_orderkey;
        }
    }

    public static class Hash1 implements HashFunction<TPCH10Orders> {
        @Override
        public int hash(TPCH10Orders key) {
    ...
    public static class Equal1 implements EqualFunction<TPCH10Orders> {
        @Override
        public boolean equal(TPCH10Orders a, TPCH10Orders b) {
