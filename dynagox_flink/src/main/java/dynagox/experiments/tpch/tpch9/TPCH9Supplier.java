...
public class TPCH9Supplier implements MultiHashMapKey {
    public int s_suppkey;
    public int s_nationkey;
    private double annotation;
    private double approximate;
    private boolean active;
    private TPCH9Supplier previous;
    private TPCH9Supplier next;

    public TPCH9Supplier() {
        this.s_suppkey = 0;
        this.s_nationkey = 0;
        this.annotation = 0.0;
    ...
    public void set_previous(MultiHashMapKey previous) {
        this.previous = (TPCH9Supplier) previous;
    }

    @Override
    public MultiHashMapKey copy() {
        TPCH9Supplier result = new TPCH9Supplier();
    ...
    public static class Hash0 implements HashFunction<TPCH9Supplier> {
        @Override
        public int hash(TPCH9Supplier key) {
            return (key.s_suppkey + 0x9e3779b9);
        }
    }

    public static class Equal0 implements EqualFunction<TPCH9Supplier> {
        @Override
        public boolean equal(TPCH9Supplier a, TPCH9Supplier b) {
            return a.s_suppkey == b.s_suppkey;
        }
    }

    public static class Hash1 implements HashFunction<TPCH9Supplier> {
        @Override
        public int hash(TPCH9Supplier key) {
    ...
    public static class Equal1 implements EqualFunction<TPCH9Supplier> {
        @Override
        public boolean equal(TPCH9Supplier a, TPCH9Supplier b) {
