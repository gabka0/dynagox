...
public class DynaGoxLine5c extends DynaGoxLine5 {
    double result = 0;

    public DynaGoxLine5c(double epsilon, boolean print) {
    ...
    public void delete_from_r1(int src, int dst) {
        k_r1.f0 = dst;
        double d_r1 = apv_r1.update_subtract_approx(k_r1, 1, b);
        if (d_r1 != 0) {
            t_r2.f0 = dst;
            LinkedListNode<Tuple2> f_r2 = r_r2.find(0, t_r2);
            while (f_r2 != null) {
    ...
    public void delete_from_r5(int src, int dst) {
        k_r5.f0 = src;
        double d_r5 = apv_r5.update_subtract_approx(k_r5, 1, b);
        if (d_r5 != 0) {
            MultiHashMap<Tuple1> d_r4s = new MultiHashMap<>(
                    new PrimaryHashIndex<>(new Tuple1.Hash(), new Tuple1.Equal()), List.of());
            t_r4.f1 = src;
