...
public class DynaGoxCEB1 extends DynaGoxCEB {
    public DynaGoxCEB1(double epsilon, boolean print) {
    ...
    public void insert_into_cast_info(int personId, int movieId) {
        ...
        if (g_mi != null) {
            k_ci.f0 = personId;
            apv_ci.update_add(k_ci, g_mi.get_approximate());
            k_pi.f0 = personId;
            Tuple1 g_pi = apv_pi.get(k_pi);
            if (g_pi != null) {
                result += g_mi.get_approximate() * g_pi.get_annotation();
            }
        }
    }
    ...
    public void insert_into_movie_info(int id, int movieId) {
        ...
        if (d_mi != 0) {
            ...
            while (f_ci != null) {
                ...
                apv_ci.update_add(k_ci, f_ci.object.get_annotation() * d_mi);
                k_pi.f0 = f_ci.object.f0;
                Tuple1 g_pi = apv_pi.get(k_pi);
                if (g_pi != null) {
                    result += f_ci.object.get_annotation() * d_mi * g_pi.get_annotation();
                }
                f_ci = f_ci.next;
            }
        }
